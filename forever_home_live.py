...
def schools_link(p): return _gsearch(f"{_place(p)} school district ratings greatschools")
def crime_link(p):   return _gsearch(f"{_place(p)} crime rate safety map")
def grow_link(p):    return _gsearch(f"{_place(p)} USDA growing zone, home vegetable garden zoning and HOA/deed restrictions, local farms CSA farmers market")
def meat_link(p):    return _gsearch(f"{_place(p)} local pasture-raised meat, farm-fresh eggs, local raw dairy, butcher, farmers market; backyard chickens and livestock zoning rules")
...
# COLOR by RELATIVE rank within surviving candidates, so the 5 colors actually spread.
# (The absolute 0-100 match score is still shown in each popup.)
N=len(props)
...
def pm(p):
    ...
    <br/><b>Area info:</b> <a href="{schools_link(p)}">🎓 Schools</a> &nbsp; <a href="{crime_link(p)}">🚔 Crime</a> &nbsp; <a href="{grow_link(p)}">🌱 Garden</a> &nbsp; <a href="{meat_link(p)}">🥚 Meat/Dairy/Eggs</a><br/>
...
desc=("Pin COLOR = match RANK among your current candidates, best to worst: "
      ...
      "The exact 0-100 match score is shown in each pin's popup. "
...
def row(p):
    area=(f"<a href='{schools_link(p)}' target='_blank'>Schools</a> · "
          f"<a href='{crime_link(p)}' target='_blank'>Crime</a> · "
          f"<a href='{grow_link(p)}' target='_blank'>Garden</a> · "
          f"<a href='{meat_link(p)}' target='_blank'>Meat/Eggs</a>")
...
def state_block(st):
    sp=[p for p in props if p["state"]==st][:25]   # props already sorted by match desc
    if not sp:
    ...
    hdr=("<tr><th>Property</th><th>Price</th><th>Type</th><th>Acres</th><th>Nearest water</th>"
         "<th>Interstate</th><th>Match</th><th>Opp</th><th>Area info</th></tr>")
...
&nbsp; <span style="color:#888">(exact 0–100 score is in each pin's popup)</span></p>
<p class="legend" style="color:#555">Showing only listings ≥ 3 acres and ≤ $300k. <b>Data check</b> — RentCast found → kept after filters: {diag}</p>
