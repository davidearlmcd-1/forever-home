...
MAX_PRICE      = 300000                       # hard ceiling
MIN_ACRES      = 3                            # hard floor
PER_QUERY      = 500
REQUEST_BUDGET = 12
...
# ---- raw-land build cost & timeline estimator (Lots / Redevelopment) ----
def build_estimate(p):
    ...
    structure=BUILD_SQFT*BUILD_PSF*m
...
def pros_cons(p):
    ...
    elif a>=5:  f.append(( 1.2, f"{a} acres — comfortably above the 3-acre floor"))
    ...
    pr=p["price"]
    if   pr<100000: f.append(( 2.0, f"${pr:,} — well under budget"))
    elif pr<200000: f.append(( 1.0, f"${pr:,} — comfortably in budget"))
    elif pr>=280000:f.append((-1.0, f"${pr:,} — near your $300k ceiling"))
    d=p["dom"]
...
for p in props:
    ...
    p["fhi"]=fhi(p); p["opp"]=opp(p); p["band"]=band(p["price"]); p["est"]=build_estimate(p)
...
def est_line(p):
    e=p.get("est")
    if not e: return ""
    return (f"<b>Est. build (land extra):</b> ${e['low']:,}–${e['high']:,} "
            f"(likely ~${e['likely']:,}) &nbsp; ~{e['months'][0]}–{e['months'][1]} mo "
            f"&nbsp;<i>confidence {e['confidence']}</i><br/>")
def anchors_line(p):
...
desc=("Pin COLOR = match RANK among your current candidates, best to worst: "
      ...
      "Exact 0-100 match score is in each pin's popup. Only listings >= 3 acres and <= $300k are shown. "
...
def row(p):
    ...
    est=p.get("est"); estcell=f"~${est['likely']:,}" if est else "—"
    ...
    return (f"<tr><td><a href='{listing_link(p)}' target='_blank'>{html.escape(str(p['address']))}</a></td>"
            f"<td>${p['price']:,}</td><td>{p['ptype']}</td><td>{p['acres']}</td>"
            f"<td>~{p['water_min']}m ({p['water_label']})</td><td>~{p['hwy_mi']}mi</td>"
            f"<td>{kincell}</td><td>{estcell}</td>"
...
def state_block(st):
    ...
    hdr=("<tr><th>Property</th><th>Price</th><th>Type</th><th>Acres</th><th>Nearest water</th>"
         "<th>Interstate</th><th>Nearest kin</th><th>Est. build</th><th>Match</th><th>Opp</th><th>Look into it</th></tr>")
...
<p class="legend" style="color:#555">Showing only listings ≥ 3 acres and ≤ $300k. <b>Data check</b> — RentCast found → kept: {diag}</p>
...
<tr><td><b>Typical raw-land build estimate</b></td><td>~${avg_build:,} <span style="color:#888;font-size:12px">(land extra; order-of-magnitude)</span></td></tr>
...
<h2>Top by State</h2>
{state_sections}
<p style="color:#888;font-size:12px">Match (Forever Home Index) auto-scores nearness to any recreational water (≤90 min), quiet (distance from interstates), acreage (diminishing returns, plateau at 15 ac), and drive time to your four anchor people. Est. build = order-of-magnitude site+structure cost for a ~1,800 sqft single-level home, land price extra. Opp = deal quality.</p>"""
