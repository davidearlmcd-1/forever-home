...
print(f"  Ceiling = ${CEILING:,}  (hide all-in above ${CEIL_MAX:,}) ({CEILING_SRC})")
...
print(f"\n{len(props)} listings kept (>= {MIN_ACRES} ac, all-in <= ${CEIL_MAX:,}): {n_fit} fit under ${CEILING:,}, {n_over} within {int((OVER_BUDGET_TOLERANCE-1)*100)}% over. Requests used: {used}/{REQUEST_BUDGET}")
...
desc=("Pin COLOR = match RANK among your current candidates, best to worst: "
      ...
      "ALL-IN cost (lot + build, or price + reno, or move-in price) slightly exceeds your ceiling is shown "
      "flagged OVER-budget in red and ranked below the ones that fit; anything more than 10% over the ceiling is hidden. "
...
<p class="legend" style="color:#555">Showing listings ≥ 2 acres with all-in ≤ ${CEIL_MAX:,}. <b>Over your ${CEILING:,} ceiling is flagged red; more than {int((OVER_BUDGET_TOLERANCE-1)*100)}% over is hidden.</b> <b>Data check</b> — found → kept: {diag}</p>
