...
def match_key(fhi_val):
    return "strong" if fhi_val>=80 else "good" if fhi_val>=65 else "moderate" if fhi_val>=50 else "weak"
MATCH_COLOR = {"strong":"ff2ca02c","good":"ff00d7ff","moderate":"ff008cff","weak":"ff3333cc"}  # aabbggrr
MATCH_LABEL = {"strong":"Strong (80+)","good":"Good (65-79)","moderate":"Moderate (50-64)","weak":"Weaker (<50)"}
...
def pm(p):
    ...
    <b>Forever Home Index (match):</b> {p['fhi']}/100 &nbsp; <b>Opportunity:</b> {p['opp']}/100<br/>
    <a href="{listing_link(p)}">🔍 Find this listing online</a>]]>"""
    return f"""<Placemark><name>{html.escape(str(p['address']).split(',')[0])} — ${p['price']:,}</name>
    <description>{d}</description><styleUrl>#m_{p['mkey']}</styleUrl>
    <Point><coordinates>{p['lon']},{p['lat']},0</coordinates></Point></Placemark>"""
styles="".join(f'<Style id="m_{k}"><IconStyle><color>{c}</color><scale>1.1</scale><Icon><href>http://maps.google.com/mapfiles/kml/shapes/homegardenbusiness.png</href></Icon></IconStyle></Style>' for k,c in MATCH_COLOR.items())
folders=""
for b in BANDS:
...
desc=("Pin color = overall match to your criteria (Forever Home Index): "
      "green strong 80+, yellow good 65-79, orange moderate 50-64, red weaker. "
      "Match rewards being near any large water (ocean, bay, sound, or major lake), acreage, price, and type. "
      "Toggle price-band folders in the sidebar to filter.")
kml=f'<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>Forever Home — Live Map</name><description>{desc}</description>{styles}{folders}</Document></kml>'
...
strong=[p for p in props if p["mkey"]=="strong"]
...
<p class="legend"><b>Pin color = overall match:</b>
<span><span class="dot" style="background:#2ca02c"></span>Strong 80+</span>
<span><span class="dot" style="background:#ffd700"></span>Good 65-79</span>
<span><span class="dot" style="background:#ff8c00"></span>Moderate 50-64</span>
<span><span class="dot" style="background:#cc3333"></span>Weaker &lt;50</span></p>
<h3>Executive Summary</h3><table>
<tr><td><b>Active listings tracked</b></td><td class="g">{len(props)}</td></tr>
<tr><td><b>Strong matches (FHI 80+)</b></td><td>{len(strong)}</td></tr>
...
<h3>Top Candidates (by overall match)</h3>
<table><tr><th>Property</th><th>Price</th><th>Type</th><th>Acres</th><th>Nearest water</th><th>Match</th><th>Opp</th></tr>{''.join(row(p) for p in top)}</table>
...
print(f"Wrote {SITE}/index.html + KMZ. Strong matches: {len(strong)}, within-1hr-water: {len(tier1)}")
