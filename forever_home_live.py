...
EXTRA_FUNDS         = 0         # add savings you'll put toward the purchase
# Cost model knobs for the all-in math:
BUILD_COST_FLOOR        = 300000  # rough min build cost; used only to fetch affordable-enough lots
...
LAND_MAXPRICE = max(50000, CEILING - BUILD_COST_FLOOR)    # lots: leave room to build
...
DROP_REASONS=["no_coords","no_lotsize","under_3ac","over_ceiling"]
def drop_reason(p):
    if not (p["lat"] and p["lon"]): return "no_coords"
    if (p["acres"] or 0)<=0:        return "no_lotsize"      # RentCast returned no lotSize
    if p["acres"]<MIN_ACRES:        return "under_3ac"
    if p["allin"]>CEILING:          return "over_ceiling"    # all-in cost exceeds home-sale ceiling
    return None
...
print(f"\n{len(props)} listings after filters (>= {MIN_ACRES} ac, all-in <= ${CEILING:,}). Requests used: {used}/{REQUEST_BUDGET}")
...
desc=("Pin COLOR = match RANK among your current candidates, best to worst: "
      ...
      "Exact 0-100 match score is in each pin's popup. Only listings >= 3 acres whose ALL-IN cost "
      "(lot + build, or price + reno, or move-in price) fits under your home-sale ceiling are shown. "
...
<p class="legend" style="color:#555">Showing only listings ≥ 3 acres whose <b>all-in cost fits under your ${CEILING:,} ceiling</b>. <b>Data check</b> — RentCast found → kept: {diag}</p>
...
<tr><td><b>Active listings that fit all-in</b></td><td class="g">{len(props)}</td></tr>
