...
# ======================================================================
STATES         = ["MD", "VA", "DE", "PA", "NC"]   # states we ACCEPT (scope, scoring, report sections)
# Broad whole-state sweeps run only on these. PA is dropped from the sweep (that's what pulled
# far-western PA / Pittsburgh) and instead covered by a focused SE-PA region query below.
QUERY_STATES   = ["MD", "VA", "DE", "NC"]
# OVERALL TOGGLE — what kind of property to search for:
#   "both"  = raw land + existing homes (lots with a house, incl. fixers)
#   "lots"  = raw land only (build from scratch)
#   "homes" = existing homes only (lots with a house, incl. fixers)
SEARCH_MODE    = "both"
_MODE_TYPES    = {"both":["Land","Single Family"], "lots":["Land"], "homes":["Single Family"]}
_MODE_LABEL    = {"both":"Raw land + existing homes", "lots":"Raw land only", "homes":"Existing homes only"}
PROPERTY_TYPES = _MODE_TYPES[SEARCH_MODE]     # lots and/or homes (redevelopment derived from homes)
MIN_ACRES      = 2                            # hard floor
PER_QUERY      = 500
REQUEST_BUDGET = 14           # per-run cap (AVM + state sweeps + region queries).
                              # ~4.3 weekly runs/mo; the cap prevents runaway. See budget note in CHANGES.
BASE           = "https://api.rentcast.io/v1/listings/sale"
AVM_URL        = "https://api.rentcast.io/v1/avm/value"

# Targeted geographic queries that MERGE INTO the same map/report (not a separate map).
# Each is one extra RentCast request focused on a lat/long + radius (miles), to guarantee
# coverage of rural areas that get crowded out of the 500-per-state slice. Add more freely.
REGION_QUERIES = [
    dict(name="SE Pennsylvania (near West Chester)", lat=40.00, lon=-75.70, radius=50, types=["Land","Single Family"]),
    dict(name="Eastern NC (Raleigh->coast)",         lat=35.40, lon=-77.40, radius=65, types=["Land","Single Family"]),
]
...
LOAN_CAPACITY       = 250000    # optional mortgage/loan ON TOP of the sale to buy/build; 0 = cash only.
                                # Each property shows how much loan it would actually need.
# Cost model knobs for the all-in math:
OVER_BUDGET_TOLERANCE   = 1.10    # show all-in up to 10% over the ceiling (near-misses); DROP anything beyond
BUILD_COST_FLOOR        = 380000  # a typical build cost; used only to bound the land query sensibly
...
def ingest_live(start_used=0):
    ...
    for st in QUERY_STATES:
        for pt in PROPERTY_TYPES:
            ...
            time.sleep(1)
    # ---- targeted region queries: merge into the SAME output list (same map + report) ----
    for rq in REGION_QUERIES:
        for pt in rq.get("types", PROPERTY_TYPES):
            if pt not in PROPERTY_TYPES:            # respect the SEARCH_MODE toggle
                continue
            if used >= REQUEST_BUDGET:
                print(f"  ! request budget ({REQUEST_BUDGET}) reached — skipping region {rq['name']}")
                return out, used
            maxp = SF_MAXPRICE if pt != "Land" else LAND_MAXPRICE
            params = {"latitude": rq["lat"], "longitude": rq["lon"], "radius": rq["radius"],
                      "propertyType": pt, "status": "Active", "maxPrice": maxp, "limit": PER_QUERY}
            try:
                r = requests.get(BASE, headers={"X-Api-Key": API_KEY}, params=params, timeout=40)
                used += 1
                if r.status_code != 200:
                    print(f"  region {rq['name']}/{pt}: HTTP {r.status_code} {r.text[:120]}"); continue
                rows = r.json(); n=0
                for x in rows:
                    xst = x.get("state")
                    if xst in STATES:
                        out.append(normalize(x, xst, pt)); n+=1
                print(f"  region {rq['name']}/{pt}: {len(rows)} found, {n} in-scope  (request {used}/{REQUEST_BUDGET})")
            except Exception as e:
                print(f"  region {rq['name']}/{pt}: ERROR {type(e).__name__} {e}")
            time.sleep(1)
    return out, used
...
def pros_cons(p):
    ...
    tier=p.get("tier","cash"); ln=p.get("loan_needed",0); ai=p.get("allin",p["price"])
    if   tier=="cash":     f.append(( 2.0, f"All-in ${ai:,} — within your ${CASH_CEILING:,} sale proceeds, no loan"))
    elif tier=="financed": f.append(( 0.3, f"All-in ${ai:,} — would need ~${ln:,} financing"))
    else:                  f.append((-2.0, f"All-in ${ai:,} — needs ~${ln:,}, beyond your ${LOAN_CAPACITY:,} loan"))
    d=p["dom"]
...
# run
# ======================================================================
print(f"Search mode: {SEARCH_MODE} — {_MODE_LABEL[SEARCH_MODE]} (querying {PROPERTY_TYPES})")
print("Determining price ceiling from your home's value...")
CASH_CEILING, CEILING_SRC, used0 = compute_ceiling()      # what the home sale (+ savings) covers, no loan
CEILING       = CASH_CEILING + LOAN_CAPACITY              # purchasing power incl. optional financing
CEIL_MAX      = int(CEILING*OVER_BUDGET_TOLERANCE)         # all-in above this is dropped (too far over budget)
SF_MAXPRICE   = CEILING                                    # homes: list price must fit under financed ceiling
LAND_MAXPRICE = max(50000, CEIL_MAX - BUILD_COST_FLOOR)    # lots: only fetch land that could land within tolerance
print(f"  Cash ceiling (home sale) = ${CASH_CEILING:,}  ({CEILING_SRC})")
print(f"  + financing up to ${LOAN_CAPACITY:,}  ->  purchasing ceiling ${CEILING:,}  (hide all-in above ${CEIL_MAX:,})")
...
for p in props:
    ...
    p["allin"]=all_in(p)
    p["loan_needed"]=max(0, p["allin"]-CASH_CEILING)
    p["tier"]=("cash" if p["allin"]<=CASH_CEILING else "financed" if p["allin"]<=CEILING else "stretch")
...
DROP_REASONS=["off_mode","no_coords","no_lotsize",UNDER_KEY,"over_budget"]
def mode_ok(pt):
    if SEARCH_MODE=="lots":  return pt=="Lot"
    if SEARCH_MODE=="homes": return pt in ("Home","Redevelopment")
    return True
def drop_reason(p):
    if not mode_ok(p["ptype"]):     return "off_mode"        # excluded by the SEARCH_MODE toggle
    ...
    if p["allin"]>CEIL_MAX:         return "over_budget"     # too far over ceiling (beyond tolerance) — hidden
    return None   # within tolerance: kept; if over the ceiling it's flagged red, else fits
...
# Cash-affordable ranks above financed, which ranks above stretch; then by match, then deal.
_rank={"cash":2,"financed":1,"stretch":0}
props.sort(key=lambda x:(_rank.get(x["tier"],0), x["fhi"], x["opp"]),reverse=True)
N=len(props)
for i,p in enumerate(props):
    q=i/max(1,N-1)
    p["mkey"]=("excellent" if q<0.2 else "strong" if q<0.4 else "moderate" if q<0.6 else "fair" if q<0.8 else "weak")
n_cash=sum(1 for p in props if p["tier"]=="cash"); n_fin=sum(1 for p in props if p["tier"]=="financed"); n_stretch=sum(1 for p in props if p["tier"]=="stretch")
print(f"\n{len(props)} listings kept (>= {MIN_ACRES} ac, all-in <= ${CEIL_MAX:,}): {n_cash} within sale proceeds, {n_fin} need financing (<=${LOAN_CAPACITY:,}), {n_stretch} stretch. Requests used: {used}/{REQUEST_BUDGET}")
...
def fund_tag(p):
    t=p["tier"]; ln=p["loan_needed"]
    if t=="cash":     return f"<span style='color:#1a9850'>within your ${CASH_CEILING:,} sale proceeds — no loan</span>"
    if t=="financed": return f"<span style='color:#b9770e'>needs ~${ln:,} financing (of ${LOAN_CAPACITY:,})</span>"
    return f"<span style='color:#c0392b'>needs ~${ln:,} — beyond your ${LOAN_CAPACITY:,} loan</span>"
def cost_line(p):
    ai=p["allin"]; fund=fund_tag(p)
    if p["ptype"]=="Lot" and p.get("est"):
        e=p["est"]
        return (f"<b>All-in: ${ai:,}</b> = lot ${p['price']:,} + build ~${e['likely']:,} "
                f"(${e['low']:,}–${e['high']:,}) &nbsp; {fund}<br/>"
                f"<span style='color:#666;font-size:12px'>~{e['months'][0]}–{e['months'][1]} mo to build · confidence {e['confidence']}</span><br/>")
    if p["ptype"]=="Redevelopment":
        return (f"<b>All-in: ${ai:,}</b> = price ${p['price']:,} + reno ~${p['reno']:,} &nbsp; {fund}<br/>")
    return f"<b>All-in: ${ai:,}</b> (move-in) &nbsp; {fund}<br/>"
def anchors_line(p):
...
def build_state_folders(plist):
    out=""
    for st in STATES:
        sprops=[p for p in plist if p["state"]==st]
        ...
        out+=f'<Folder><name>{st} ({len(sprops)})</name><open>0</open>{msub}</Folder>'
    return out
# TOP-LEVEL TOGGLE: uncheck a type folder in Google Earth to hide all of it instantly.
TYPE_GROUPS=[("\U0001F3E1 Existing homes", ("Home","Redevelopment")), ("\U0001F332 Raw land", ("Lot",))]
folders=""
for tname, tset in TYPE_GROUPS:
    tprops=[p for p in props if p["ptype"] in tset]
    if not tprops: continue
    folders+=f'<Folder><name>{tname} ({len(tprops)})</name><open>1</open>{build_state_folders(tprops)}</Folder>'
desc=("Pin COLOR = match RANK among your current candidates, best to worst: "
      ...
      "Exact 0-100 match score is in each pin's popup. Listings >= 2 acres are shown. Each popup shows the "
      "ALL-IN cost (lot + build, price + reno, or move-in price) and its funding: within your home-sale proceeds "
      "(no loan), needing financing up to your loan cap, or beyond it. Anything past ~10% over the financed ceiling is hidden. "
      "TO FILTER: sidebar checkboxes, nested Type (Existing homes / Raw land) > State > Match tier > Price band. "
      "Uncheck 'Raw land' or 'Existing homes' at the top to instantly see only the other. "
...
def row(p):
    ...
    t=p["tier"]; ln=p["loan_needed"]
    tag=("<span style='color:#1a9850'>cash</span>" if t=="cash"
         else f"<span style='color:#b9770e'>+${ln:,} loan</span>" if t=="financed"
         else f"<span style='color:#c0392b'>+${ln:,}!</span>")
    aicell=f"${p['allin']:,}<br><span style='font-size:11px'>{tag}</span>"
    rtype="land" if p["ptype"]=="Lot" else "home"
    return (f"<tr data-type='{rtype}'><td><a href='{listing_link(p)}' target='_blank'>{html.escape(str(p['address']))}</a></td>"
...
def state_block(st):
    ...
    hdr=("<tr><th>Property</th><th>List price</th><th>All-in (funding)</th><th>Type</th><th>Acres</th><th>Nearest water</th>"
...
.tf{{margin-right:6px;padding:5px 11px;border:1px solid #ccc;background:#fff;border-radius:6px;cursor:pointer;font-size:13px}}.tf.on{{background:#2ca02c;color:#fff;border-color:#2ca02c}}
...
<h1>Forever Home — Weekly Report</h1><p style="color:#666">{today} · MD·VA·DE·PA·NC · <b>Mode: {_MODE_LABEL[SEARCH_MODE]}</b> · live RentCast data · updates every Monday</p>
<p><a class="btn" href="{kmz_link}">⬇ Download map (.kmz)</a><a class="btn" href="{nl_link}">\U0001F30E Auto-refresh map for Google Earth</a></p>
<p class="typefilter"><b>Show:</b> <button onclick="ft('all',this)" class="tf on">All</button><button onclick="ft('home',this)" class="tf">Existing homes only</button><button onclick="ft('land',this)" class="tf">Raw land only</button></p>
<script>function ft(t,b){{document.querySelectorAll('tr[data-type]').forEach(function(r){{r.style.display=(t=='all'||r.dataset.type==t)?'':'none';}});document.querySelectorAll('.tf').forEach(function(x){{x.classList.remove('on');}});if(b)b.classList.add('on');}}</script>
...
<p class="legend" style="color:#555">Showing listings ≥ 2 acres. Funding tag: <b style="color:#1a9850">cash</b> = within your ${CASH_CEILING:,} sale proceeds · <b style="color:#b9770e">+loan</b> = needs financing (≤${LOAN_CAPACITY:,}) · <b style="color:#c0392b">red</b> = beyond even the loan (hidden past {int((OVER_BUDGET_TOLERANCE-1)*100)}% over). <b>Data check</b> — found → kept: {diag}</p>
...
<tr><td><b>Cash ceiling (home sale + savings)</b></td><td class="g">${CASH_CEILING:,}</td></tr>
<tr><td><b>+ financing considered</b></td><td>up to ${LOAN_CAPACITY:,} → purchasing ceiling <b>${CEILING:,}</b></td></tr>
...
<tr><td><b>&nbsp;&nbsp;· within proceeds / need financing / stretch</b></td><td>{n_cash} / {n_fin} / {n_stretch}</td></tr>
...
<h2>Top by State</h2>
{state_sections}
<p style="color:#888;font-size:12px">Cash ceiling = your home's estimated sale value (+ savings); financed ceiling adds up to ${LOAN_CAPACITY:,} of optional loan. Every property's <b>all-in</b> cost (lot + build for a 3-4 br house, price + reno for a fixer, or list price for a move-in home) is compared to both, and tagged with how much loan (if any) it would take. A loan is debt carried into retirement — the funding tag is to inform, not to recommend borrowing. Match auto-scores water (≤90 min), quiet, acreage (plateau at 15 ac), and drive time to your four anchor people. Opp = deal quality.</p>"""
