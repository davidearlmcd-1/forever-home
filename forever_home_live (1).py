...
# ONE pin shape for everything (a clean colored dot) — color carries the meaning
...
styles="".join(f'<Style id="m_{k}"><IconStyle><color>{c}</color><scale>1.2</scale><Icon><href>{ICON}</href></Icon></IconStyle></Style>' for k,c in MATCH_COLOR.items())
# Nested folders so you can filter in Google Earth: Type -> Price band. Toggle checkboxes to show/hide.
TYPE_GROUPS=[("Homes (house on the land)", lambda p: p["ptype"] in ("Home","Redevelopment")),
             ("Land only (no house)",       lambda p: p["ptype"]=="Lot")]
folders=""
for gname,gtest in TYPE_GROUPS:
    gprops=[p for p in props if gtest(p)]
    if not gprops: continue
    sub=""
    for b in BANDS:
        pms="".join(pm(p) for p in gprops if p["band"]==b); n=sum(1 for p in gprops if p["band"]==b)
        if pms: sub+=f'<Folder><name>{b}  ({n})</name><open>0</open>{pms}</Folder>'
    folders+=f'<Folder><name>{gname}  ({len(gprops)})</name><open>0</open>{sub}</Folder>'
desc=("Pin COLOR = overall match (Forever Home Index), best to worst: "
      ...
      "TO FILTER: in the sidebar expand 'Homes' or 'Land only', then tick/untick the price-band boxes to show or hide groups. "
